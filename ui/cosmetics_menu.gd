...
@onready var preview_area = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/PreviewArea
@onready var preview_underlay = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/PreviewArea/BackgroundUnderlay
@onready var cosmetic_sets_list = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/PreviewArea/PreviewMargin/PreviewVBox/CosmeticSetsList
@onready var playercard_option = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/PreviewArea/PreviewMargin/PreviewVBox/TopBar/PlayerCardBox/PlayerCardOption
@onready var playercard_preview = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/PreviewArea/PreviewMargin/PreviewVBox/TopBar/PlayerCardBox/PlayerCardPreview
@onready var ingame_bg_option = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/PreviewArea/PreviewMargin/PreviewVBox/TopBar/BackgroundDropdownBox/IngameBackgroundOption
@onready var charsel_bg_option = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/CharSelectSection/CharSelectRow/CharSelectBackgroundOption
@onready var charsel_bg_preview = $CosmeticsMenu/ContentMargin/MainVBox/BodyScroll/BodyVBox/CharSelectSection/CharSelectRow/CharSelectBackgroundPreview
@onready var close_button = $CloseMargin/CloseButton
...
func _populate_from_player():
	...
	for i in range(3):
		...
		cosmetic_sets_list.add_child(cosmetic_set)
...
func ingest_backgrounds():
	...
	for background in ingame_backgrounds:
		...
		ingame_bg_option.add_item(cosmetic_name, index)
		if _player.equipped_ingame_background == background:
			ingame_bg_option.selected = index
			_set_preview_background(_player.equipped_ingame_background)
		index += 1
	index = 1
	for background in ingame_backgrounds:
		var cosmetic_info = background.split("_")
		var cosmetic_name = cosmetic_info[2].capitalize()
		charsel_bg_option.add_item(cosmetic_name, index)
		if _player.equipped_charselect_background == background:
			charsel_bg_option.selected = index
			charsel_bg_preview.texture = load("res://assets/backgrounds/" + _player.equipped_charselect_background + ".png")
		index += 1


func _set_preview_background(bg_name: String):
	if preview_underlay:
		preview_underlay.texture = load("res://assets/backgrounds/" + bg_name + ".png")
...
func ingest_playercards():
	...
	for playercard in playercards:
		...
		var cosmetic_name = cosmetic_info[2].capitalize()
		playercard_option.add_item(cosmetic_name, index)
		if _player.equipped_player_card == playercard:
			playercard_option.selected = index
			playercard_preview.texture = load("res://assets/cosmetics/playercards/" + _player.equipped_player_card + ".png")
		index += 1
...
func settle_chosen_cosmetics():
	...
	for cosmetic_set in cosmetic_sets_list.get_children():
...
func _on_texture_button_mouse_entered():
	close_button.modulate = Color.hex(0xffffff88)
	play_sound("hover")
