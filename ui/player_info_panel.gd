...
static func from_player(player):
	var panel = load("res://ui/player_info_panel.tscn").instantiate()
...
func assign_player(player):
	...
	rank_label.text = player.rank.to_str() + " (" + str(int(player.rank._rp)) + " RP)"
	clan_label.text = "Clan: " + player.clan
	if not player.title == "":
		ladder_label.text = player.title
	var ranked_streak_string = "Rating: " + str(int(player.rank.get_rating()))
	...
	if player.rank.streak >= 0:
		...
		personal_streak_string = personal_streak_string + str(int(player.rank.streak))
	var ratio_string = "Ratio: " + str(int(player.rank.wins)) + " - " + str(int(player.rank.losses))
	if player.rank.streak >= 0:
...
func avatar_accept_click(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == 1:
		...
		avatar_request.emit($AvatarUploadPane/MarginContainer/VBoxContainer/LineEdit.text)
