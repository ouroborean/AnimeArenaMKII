...
signal accept_clan_invitation(clan_name, player)
signal decline_clan_invitation(clan_name, player)
...
func ingest_details(clan_details):
	...
	$MarginContainer/HBoxContainer/Label2.text = "(" + str(clan_details["members"]) + ")"
...
func apply_click(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == 1:
		decline_clan_invitation.emit(clan_name, "")
...
func view_click(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == 1:
		accept_clan_invitation.emit(clan_name, "")
