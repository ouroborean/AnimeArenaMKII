...
func describe(user):
	...
	return "For 1 turn, counters the first Mental skill used by one enemy and swaps to 'Air Wave'. Invisible."

func execute(user, battle):
	var context = QueryContext.from_game_state(user, battle)
	for target in user.targeter.targets:
		var counter = Effect.counter_effect(Trigger.always(default_counter_trigger), EffectType.Type.COUNTER_USE, 2, "The first Mental skill used by this character will be countered.", ["Mental"])
	...
	var abi_swap = Effect.ability_swap_effect(5, 1, user, 3)
