...
func split_desc():
	return [
		...
		["Silences for 1 turn if damaged by Projection - Kanshou last turn", Color.CADET_BLUE]
	]
...
func execute(user, battle):
	...
	for target in user.targeter.targets:
		...
		var mark = Effect.mark(3, "Projection - Kanshou will Stun this character for 1 turn.")
		mark.set_source(self)
		if target.marked_by("Projection - Kanshou"):
			
			user.manually_advance_mission(7, 1)
			var stun = Effect.silence_effect(2)
