...
func describe(user):
	...
	return "For 1 turn, counters the first Affliction skill used by one enemy and swaps to 'Fire Fist'. Invisible."

func execute(user, battle):
	var context = QueryContext.from_game_state(user, battle)
	for target in user.targeter.targets:
		var counter = Effect.counter_effect(Trigger.always(default_counter_trigger), EffectType.Type.COUNTER_USE, 2, "The first Affliction skill used by this character will be countered.", ["Affliction"])
	...
	var abi_swap = Effect.ability_swap_effect(4, 0, user, 3)
