...
func split_desc():
	return [
		...
		["Will trigger twice if the target is Stunned", Color.CADET_BLUE]
	]
...
func execute(user, battle):
	...
	for target in user.targeter.targets:
		...
		if target.has_stuns():
			Character.resolve_damage(context, target, 10, DamageType.Type.NORMAL)
			var sub_mark = Effect.mark(-1, func (eff): return "Hawkmon has " + str(eff.stacks) + " stacks of Hawkmon Digivolve.")
			sub_mark.stackable = true
			sub_mark.display_stacks = true
			sub_mark.set_source(user.moveset.base_abilities[2])
